...
SRC_FILES := $(shell find $(SRC_DIR) -name "*.c" -or -name "*.cpp" -or -name "*.cc")
SRCS := $(FB_SRCS) $(SRC_FILES)
OBJS := $(patsubst $(SRC_DIR)/%.c, $(BUILD_DIR)/%.c.o, $(SRC_FILES))
OBJS := $(patsubst $(SRC_DIR)/%.cpp, $(BUILD_DIR)/%.cpp.o, $(OBJS))
OBJS := $(patsubst $(SRC_DIR)/%.cc, $(BUILD_DIR)/%.cc.o, $(OBJS))
OBJS += $(patsubst $(BUILD_DIR)/%.cpp, $(BUILD_DIR)/%.cpp.o, $(FB_SRCS))
